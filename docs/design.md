...
Chat arrivals go through `ChatEntryMotion` and play on mount. Assistant text does not fade as a row; newly revealed words fade on the burst window without restarting when the line grows. Permissions and the turn footer wrapper stay at rest. Copy, fork, and elapsed insides stagger only when a live turn just completed. History paints complete. Streaming growth clips new lines on the arrive window so the live footer can slide with the block until the viewport overflows. A whole-row insert (a sent message) keeps arrive — it is not a stream lump. Timeline rise uses burst only for in-flight catch-up. Tool-call summary labels ease width and fade the new suffix when counts grow. The loading shimmer keeps its original sweep so extra words do not flash. Files and Changes disclosure chevrons use the same arrive/exit curves.

Expand/collapse uses one resize policy (`resolveCollapseClipResize`) for height (`ExpandableBadgeCollapseClip`) and width (`ExpandWidthClip`). Growth clips (chat wrap, summary labels) share `applyGrowthSize`. Web measurement uses `useObservedSize`. Arrive fade+rise uses `resolveArriveFadeStyle`. Content inside a badge rises 4px after a 40ms delay. The left workspace list and the Explorer dock ease width on those same arrive/exit curves. First paint snaps. Drag-resize snaps. Inner content keeps the open width so labels do not reflow in the clip.
