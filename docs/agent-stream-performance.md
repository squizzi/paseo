...
- **Chat arrivals share one motion boundary.** Use `chat-entry-motion.tsx` for stream rows, live auxiliary rows, and markdown blocks inside a streaming assistant message. Motion starts when the element intersects the chat viewport; a React mount below the fold would otherwise finish off-screen and read as a pop. If streaming eligibility ends before that intersection, snap to rest — otherwise the observer disconnects with progress still at zero and the completed row stays transparent when the user later scrolls to it. Assistant rows do not also animate as a unit — that double-fades the first block and still leaves later paragraphs popping in. Wrapped lines grow inside an already-mounted block, so `ChatGrowthClip` eases that block's height and clips the new line until it rises in. Streaming Thinking and other plain-text tool-call details reuse that clip; a layout transition on the expanded badge would fight it. File-edit diffs and other non-plain details use the same clip while the call is running, and web eases a completed expand from zero height so those lines do not pop. User-submitted rows animate on first appearance after history hydration, not only while the send is pending — a fast provider ack would otherwise remount the row at full opacity. Overview tool calls reuse their first row as later calls arrive, so the row revision is the latest grouped call ID; a mount-only animation misses every follow-up. Follow-up calls inside an expanded overview clip and enter like other chat arrivals.
