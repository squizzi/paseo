...
- **Chat arrivals share one motion boundary.** Use `chat-entry-motion.tsx` for user-sent rows and new streaming thought/tool rows as they appear. Motion starts when the element intersects the chat viewport; a React mount below the fold would otherwise finish off-screen and read as a pop. If streaming eligibility ends before that intersection, snap to rest — otherwise the observer disconnects with progress still at zero and the completed row stays transparent when the user later scrolls to it. Assistant text, permissions, and the turn footer do not fade. Wrapped lines grow inside an already-mounted block, so `ChatGrowthClip` eases that block's height and clips the new line until it rises in. Streaming Thinking and other plain-text tool-call details reuse that clip; a layout transition on the expanded badge would fight it. File-edit diffs and other non-plain details use the same clip while the call is running, and expandable badges ease both expansion and collapse smoothly so details do not pop or snap shut (details already open during streaming do not replay when execution settles). User-submitted rows animate on first appearance after history hydration, not only while the send is pending — a fast provider ack would otherwise remount the row at full opacity. Follow-up calls inside an expanded overview use the growth clip. They do not fade in.
