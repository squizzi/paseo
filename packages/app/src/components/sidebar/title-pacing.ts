/**
 * Character-paced reveal for sidebar title type-in.
 *
 * Assistant text uses word-stream. Titles still type in by character because a
 * rename is a replacement, not growth of the same string.
 *
 * Everything here is pure. The React hook adds a frame clock so the policy stays
